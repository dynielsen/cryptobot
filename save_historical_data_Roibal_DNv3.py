...
def save_historic_klines_csv(symbol, start, end, interval):
    ...
    with open('Binance_{}_{}-{}.txt'.format(symbol, start, end), 'w') as f:
        ...
        for kline in klines:
            ...
            ochl.append([time1, open1, High, Low, Close, Volume])
            f.write(format_kline)
            
            data = [time1, open1, Low, High, Close, Volume] 
        ...
        df = pd.DataFrame(data_all, columns = ['Time', 'Open', 'Low','High','Close','Volume']) 
    ...
    ax.set(xlabel='Date', ylabel='Price [USDT]', title='{} {}-{}'.format(symbol, start, end))
    plt.legend()
    plt.grid()
    plt.savefig((symbol + '-plot.png'))
    plt.show()
    return df
